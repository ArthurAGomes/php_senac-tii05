...
class DatabaseRepository {
    ...
    public static function insertContact($nome, $quantidade, $comprado) {
        $pdo = self::connect();
        $sql = "INSERT INTO itens_compra (nome_produto, quantidade, comprado) 
                VALUES (:nome_produto, :quantidade, :comprado)";
        $stmt = $pdo->prepare($sql);
        return $stmt->execute(['nome_produto' => $nome, 'quantidade' => $quantidade, 'comprado' => $comprado]);
    }

    public static function updateContact($id, $nome, $quantidade, $comprado) {
        $pdo = self::connect();
